...
#     6:  cutout
#     else:  翻转
#     可进行任意组合，只需把操作的序号加入列表传入dataAugment()中即可
...
class DataAugmentForObjectDetection():
    def __init__(self, rotation_rate=0.5, max_rotation_angle=5, 
                ...
                cutout_rate=0.5, cut_out_length=150, cut_out_holes=3, cut_out_threshold=1.5):
    ...
    def _changeLight(self, img):
        # random.seed(int(time.time()))
        flag = random.uniform(0.5, 1.4)     #flag>1为调暗,小于1为调亮
        return exposure.adjust_gamma(img, flag)
    
    # cutout
    def _cutout(self, img, bboxes, length=100, n_holes=3, threshold=1):
        '''
    ...
    def dataAugment(self,img,bboxes,types,img_path, xml_path, img_save_path, xml_save_path):
        '''
            图像增强
            输入:
                img:图像array
        ...
        #print(img_name)
        for type in types:
            name = name + '_' + str(type)
            if type == 1:  # 裁剪
                img, bboxes = self._crop_img_bboxes(img, bboxes)
            elif type == 2: #旋转
                angle = random.sample([60, 90, 120, 150, 210, 240, 300],1)[0]
        ...
        if s == ".jpg":
            ...
            for box in root.iter('bndbox'):
                    # change the coord
                box.find('xmin').text = str(bboxes[0][0])
                box.find('ymin').text = str(bboxes[0][1])
                box.find('xmax').text = str(bboxes[0][2])
                box.find('ymax').text = str(bboxes[0][3])
                # write into new xml
            tree.write(xml_save_path + n  + name + ".xml")
...
if __name__ == '__main__':
    ...
    source_pic_root_path = 'D:\ImageDataSet\smokeTest'
    source_xml_root_path = 'D:\ImageDataSet\smokeTestXML'
    out_pic_root_path = 'D:\ImageDataSet\\rotateSmokeTest\\'
    out_xml_root_path = 'D:\ImageDataSet\\rotateSmokeTestXML\\'

    for img_name in os.listdir(source_pic_root_path):
        pic_path = os.path.join(source_pic_root_path, img_name)
        xml_path = os.path.join(source_xml_root_path, img_name[:-4] + '.xml')
        coords_list = parse_xml(xml_path)
        coords = [coord[:4] for coord in coords_list]
        ...
        # else:  翻转
        #operate = [8]     #执行翻转操作
        #operate = [1,2,3]    #同时执行裁剪、旋转、平移操作
        num = random.randint(2,4)
        operate = random.sample(range(1,8),num);   #随机组合转换操作
        print(operate)
        dataAug.dataAugment(img, coords, operate, pic_path, xml_path, out_pic_root_path, out_xml_root_path)
